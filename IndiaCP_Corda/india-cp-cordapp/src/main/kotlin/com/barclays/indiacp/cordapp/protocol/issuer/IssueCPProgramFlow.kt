...
class IssueCPProgramFlow(val contractState: IndiaCommercialPaperProgram.State) : FlowLogic<SignedTransaction>() {
    ...
    fun getTracker() = ProgressTracker(CREDIT_LIMIT_CHECK, CREDIT_VALIDITY_CHECK, BORROWING_LIMIT_CHECK, SELF_ISSUING, OBTAINING_NOTARY_SIGNATURE, NOTARY_SIGNATURE_OBTAINED, RECORDING_TRANSACTION, TRANSACTION_RECORDED)
...
//        val states = this.serviceHub.vaultService.linearHeadsOfType<IndiaCommercialPaperProgram.State>()
//        val deals = states?.values.map { it }
//        val cp = deals ?: if(deals.isNotEmpty()) deals[0] else null
//
//        val creditRatingStateAndRef: StateAndRef<CreditRating.State>
//        val states = serviceHub.vaultService.currentVault.statesOfType<BorrowingLimitBoardResolution.State>()
...
//        } else {
//            creditRatingStateAndRef = states[0]
//        }

//        if (creditRatingStateAndRef == null) {
//            throw IndiaCPException(CreditRatingError.DOES_NOT_EXIST_ERROR, Error.SourceEnum.DL_R3CORDA, "Credit Rating Document for this Legal Entity Not Found. The Legal Entity Should be setup with the Credit Rating and Board Resolution Documents before Initiating a CP Program.")
//        }
...
//        val boardResolutionStateAndRef = CPUtils.getContractStateAndRef<BorrowingLimitBoardResolution.State>(serviceHub)
//        if (boardResolutionStateAndRef == null) {
...
//        val tx = IndiaCommercialPaperProgram().generateIssue(contractState, creditRatingStateAndRef!!, boardResolutionStateAndRef!!, notary = notary.notaryIdentity)
        ...
        val tx = IndiaCommercialPaperProgram().generateIssue(contractState, notary = notary.notaryIdentity)
    ...
    private fun netCPPrograms() : Pair <Amount<Currency>, Amount<Currency>> {
        ...
        return Pair<Amount<Currency>, Amount<Currency>>(Amount(0,Currency.getInstance("INR")), Amount(0,Currency.getInstance("INR")))
    }
